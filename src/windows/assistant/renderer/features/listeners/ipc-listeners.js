export function setupIpcListeners({
    ...
    if (windowApi.onTriggerExplainDetail && typeof explainPreviousAnswerInDetail === 'function') {
        windowApi.onTriggerExplainDetail(() => {
            addMonitorLog('info', 'shortcut-event', 'Explain-in-Detail shortcut triggered');
            explainPreviousAnswerInDetail().catch((error) => {
                console.error('Explain-in-Detail trigger failed:', error);
                addMonitorLog('error', 'shortcut-explain-detail-failed', error.message);
            });
        });
    }
