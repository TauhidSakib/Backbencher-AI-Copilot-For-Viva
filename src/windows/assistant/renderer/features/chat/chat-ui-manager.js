export function createChatUiManager({
    ...
    `;
        ...
        // When auto-scroll is ON, always follow to the bottom (the toggle is the
        // user's explicit intent). The old `isChatNearBottom` gate made it stop
        // following mid-stream once content grew past the 28px window.
    ...
    function submitManualContextMessage() {
        ...
        // A message typed into the chat box is a QUESTION/instruction for the AI
        // to answer, so record it as a Host line ("Host: …"). The Ask AI prompt
        // answers the latest Host utterance — recording it as "voice-mic" (a
        // "You:" line) meant typed follow-ups were treated as the candidate's own
        // words and ignored, so the AI kept re-answering the earlier question.
        addChatMessage('voice-system', text);
        addMonitorLog?.('info', 'manual-question-added', 'Typed question added', 'system', {
            chars: text.length
        });
        showFeedback?.('Question added — press Ask AI', 'success');
