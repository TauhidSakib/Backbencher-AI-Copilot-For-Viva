...
function setAutoScrollEnabled(value) {
    ...
    // Turning auto-scroll ON should immediately jump to the latest message.
    if (autoScrollEnabledState && chatMessagesElement) {
...
function paintMobileServerPill() {
    ...
    lines.push('  New-NetFirewallRule -DisplayName "Backbencher AI Copilot Mobile" -Direction Inbound -LocalPort 7823 -Protocol TCP -Action Allow -Profile Any');
...
const settingInterviewTopic = document.getElementById('setting-interview-topic');
...
function applyApiKeyAvailabilityFromSettings() {
    // The app runs entirely on local engines that need no API key:
    // Ollama for AI answers and offline Vosk for transcription. So both
    // capabilities are always available.
    hasGeminiApiKeysConfigured = true;
    hasAssemblyAiApiKeyConfigured = true;
}
...
function buildAskAiContextPayload() {
    // Commit any transcript segments still held in the merge buffer so the
    // interviewer's most recent question is included in the context.
    flushAllFinalTranscripts('pre-ask-ai');
    const bundle = buildFilteredAiContextBundle({ charBudget: AI_CONTEXT_CHAR_BUDGET, emitTruncationLog: true });

    // CRITICAL: Vosk only emits a "final" after a pause, so the interviewer's
    // newest question is frequently still a LIVE PARTIAL when Ask AI is pressed.
    // If we ignore it, the model answers the PREVIOUS finalized question. So we
    // append the current partials as the latest lines — the system (Host) partial
    // becomes the current question that the prompt will answer.
    let transcriptContext = bundle.transcriptContext;
    let contextString = bundle.contextString;
    const partials = transcriptionManager.getLivePartials
        ? transcriptionManager.getLivePartials()
        : { system: '', mic: '' };
    const micPartial = String(partials.mic || '').trim();
    const systemPartial = String(partials.system || '').trim();
    // Order matters: append the interviewer (Host) partial LAST so it is the most
    // recent Host line and is picked as the current question.
    if (micPartial) {
        transcriptContext = `${transcriptContext}\nYou: ${micPartial}`.trim();
        contextString = `${contextString}\nYou: ${micPartial}`.trim();
    }
    if (systemPartial) {
        transcriptContext = `${transcriptContext}\nHost: ${systemPartial}`.trim();
        contextString = `${contextString}\nHost: ${systemPartial}`.trim();
    }
...
async function askAiWithSessionContext() {
    if (!hasGeminiApiKeysConfigured) {
        showFeedback('AI is unavailable. Make sure Ollama is running.', 'error');
        return;
    }
...
// Alt+Shift+D — re-answer my previous answer with full, in-depth detail.
// Injects an explicit "explain in detail" turn as the current question, then
// runs the normal Ask AI pipeline: the detail intent raises the token budget,
// and my previous answer is available to the model via the session summary.
async function explainPreviousAnswerInDetail() {
    addChatMessage('voice-system', 'Explain your previous answer in complete, in-depth detail — step by step, with the full reasoning, concrete examples, trade-offs, and complete code if relevant.');
...
function clearChatUiForSession() {
    screenshotsCount = 0;
    messageStore.clear();
    chatMessagesArray = messageStore.getMessages();
...
// Session-scoped document upload (RAG). Files live only in the current session.
function setupSessionDocs() {
    const attachBtn = document.getElementById('attach-file-btn');
    const fileInput = document.getElementById('file-upload-input');
    const filesBar = document.getElementById('session-files-bar');
    const docCount = document.getElementById('session-doc-count');
    const newSessionBtn = document.getElementById('new-session-btn');
    const endSessionBtn = document.getElementById('end-session-btn');
    const composer = document.getElementById('chat-composer');
    const api = window.electronAPI || {};

    if (!filesBar || typeof api.ragGetSession !== 'function') return;

    let files = [];

    function renderFiles(pendingNames = []) {
        filesBar.innerHTML = '';
        const hasAny = files.length > 0 || pendingNames.length > 0;
        filesBar.classList.toggle('hidden', !hasAny);
        if (docCount) docCount.textContent = `${files.length} doc${files.length === 1 ? '' : 's'}`;
        ...
        files.forEach((file) => {
            ...
            name.title = `${file.name} · ${file.chunks} chunk(s)`;
            ...
            remove.addEventListener('click', async () => {
                const res = await api.ragRemoveFile(file.fileId);
                files = (res && res.files) || [];
                renderFiles();
                showFeedback('Document removed', 'info');
            });
        ...
        pendingNames.forEach((pendingName) => {
            const chip = document.createElement('span');
            chip.className = 'file-chip pending';
            const name = document.createElement('span');
            name.className = 'file-chip-name';
            name.textContent = pendingName;
    ...
    async function uploadFiles(fileList) {
        ...
        for (const file of incoming) {
            renderFiles([file.name]);
            showFeedback(`Indexing ${file.name}…`, 'info');
            try {
                const base64 = await fileToBase64(file);
                const res = await api.ragUploadFile(file.name, base64);
                if (res && res.success) {
                    files = res.files || files;
                    showFeedback(`Added ${file.name}`, 'success');
                } else {
                    showFeedback(`Upload failed: ${(res && res.error) || 'unknown error'}`, 'error');
                }
            } catch (error) {
    ...
    if (newSessionBtn) {
        newSessionBtn.addEventListener('click', async () => {
            const res = await api.ragNewSession();
            files = (res && res.files) || [];
            renderFiles();
            clearChatUiForSession();
            showFeedback('New session started', 'success');
        });
    }

    if (endSessionBtn) {
        endSessionBtn.addEventListener('click', async () => {
            const res = await api.ragEndSession();
            files = (res && res.files) || [];
            renderFiles();
            clearChatUiForSession();
            showFeedback('Session ended — all uploaded documents permanently deleted', 'success');
        });
    }
    ...
    if (composer) {
        ...
        ['dragleave', 'dragend'].forEach((eventName) => {
            composer.addEventListener(eventName, () => composer.classList.remove('drag-over'));
        });
    ...
    if (typeof api.onSessionReset === 'function') {
        api.onSessionReset(() => {
            files = [];
            renderFiles();
            clearChatUiForSession();
