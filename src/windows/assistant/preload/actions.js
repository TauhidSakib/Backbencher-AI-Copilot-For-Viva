...
function createInvokeActions(ipcRenderer) {
  return {
    ...
    ragGetSession: invokeWithFallback(ipcRenderer, {
      channel: 'rag-get-session',
      label: 'ragGetSession',
      fallback: (error) => ({ sessionId: null, files: [], error: error.message })
    }),

    ragUploadFile: invokeWithFallback(ipcRenderer, {
      channel: 'rag-upload-file',
      label: 'ragUploadFile',
      transformArgs: (args) => [{ name: args[0], dataBase64: args[1] }],
      fallback: (error) => ({ success: false, error: error.message, files: [] })
    }),

    ragRemoveFile: invokeWithFallback(ipcRenderer, {
      channel: 'rag-remove-file',
      label: 'ragRemoveFile',
      fallback: (error) => ({ success: false, error: error.message, files: [] })
    }),

    ragNewSession: invokeWithFallback(ipcRenderer, {
      channel: 'rag-new-session',
      label: 'ragNewSession',
      fallback: (error) => ({ success: false, error: error.message, files: [] })
    }),

    ragEndSession: invokeWithFallback(ipcRenderer, {
      channel: 'rag-end-session',
      label: 'ragEndSession',
      fallback: (error) => ({ success: false, error: error.message, files: [] })
    })
  };
