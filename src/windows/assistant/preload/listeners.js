...
function createEventActions(ipcRenderer) {
  ...
  const onTriggerExplainDetail = createEventListener(ipcRenderer, {
    channel: 'trigger-explain-detail',
    label: 'onTriggerExplainDetail'
  });
  ...
  const onSessionReset = createEventListener(ipcRenderer, {
    channel: 'session-reset',
    label: 'onSessionReset'
  });
