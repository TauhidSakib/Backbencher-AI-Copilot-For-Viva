...
function sanitizeAppState(state) {
  ...
  if (state && typeof state === 'object' && !Array.isArray(state)) {
    ...
    // Interview / viva topic (free text). Capped so it can never blow up the
    // prompt / context window.
    if (typeof state.interviewTopic === 'string' && state.interviewTopic.trim()) {
      nextState.interviewTopic = state.interviewTopic.trim().slice(0, 600);
    }
