...
function createWindowController({
  ...
  function emergencyHide() {
    // Any pending auto-hide timer from older behaviour is cancelled — this is a
    // pure manual TOGGLE now.
    if (autoHideTimer) {
      clearTimeout(autoHideTimer);
      autoHideTimer = null;
    }
    ...
    // Toggle visibility. Pressing the shortcut hides the window and it STAYS
    // hidden until the SAME shortcut is pressed again — no automatic re-show.
    if (isVisible) {
      isVisible = false;
  ...
  function registerShortcuts() {
    ...
    registerShortcut('explainDetail', () => {
      sendToRenderer('trigger-explain-detail');
    });
